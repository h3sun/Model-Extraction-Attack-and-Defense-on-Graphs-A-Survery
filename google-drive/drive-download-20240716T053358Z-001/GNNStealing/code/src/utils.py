...
def load_data(dataset_name):  # TODO not citeseer_full, it should contain many graphs
    """
...
def split_graph(g, frac_list=[0.6, 0.2, 0.2]):  # TODO graph should be a list!
    g_list = g.ndata['features'].numpy().tolist()
    train_subset, val_subset, test_subset = dgl.data.utils.split_dataset(
        g_list, frac_list=frac_list, shuffle=True)
...
def split_graph_different_ratio(g, frac_list=[0.6, 0.2, 0.2], ratio=0.5):  # TODO graph should be a list
    g_list = g.ndata['features'].numpy().tolist()
    train_subset, val_subset, test_subset = dgl.data.utils.split_dataset(
        g_list, frac_list=frac_list, shuffle=True)
